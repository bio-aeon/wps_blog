...
class PageServiceSpec extends Specification {
  ...
  "PageService.getPageByUrl" >> {
    ...
    "transforms Page domain model to PageResult API model" >> {
  ...
  "PageService.getAllPages" >> {
    "returns all pages as ListPageResult items" >> {
      val pages = List(
    ...
    "transforms Page domain to ListPageResult with url and title only" >> {
      val pages = List(
        Page(
          "privacy",
          "Privacy Policy",
          "Long privacy content",
          ZonedDateTime.now(),
          Some(PageId(1))
        )
      )
