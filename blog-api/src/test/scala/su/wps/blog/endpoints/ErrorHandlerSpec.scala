...
class ErrorHandlerSpec extends Specification with CatsEffect {
  ...
  "ErrorHandler" >> {
    "returns 404 for PostNotFound error" >> {
      ...
      ErrorHandler(routes).run(request).value.map(_.get).map { resp =>
    ...
    "includes post id in NotFound response body for PostNotFound" >> {
      ...
      for {
        resp <- ErrorHandler(routes).run(request).value.map(_.get)
        body <- resp.as[String]
      } yield {
