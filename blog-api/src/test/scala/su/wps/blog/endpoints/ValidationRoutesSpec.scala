...
class ValidationRoutesSpec extends Specification with RoutesSpecSupport with CatsEffect {
  ...
  "GET /posts" >> {
    ...
    "rejects non-numeric limit" >> {
      ...
      val request = Request[IO](
        Method.GET,
        Uri.unsafeFromString(s"$v1/posts").withQueryParams(Map("limit" -> "abc", "offset" -> "0"))
      )
      ...
      for {
        ...
        resp.status mustEqual Status.BadRequest
        body must contain("\"code\":\"BAD_REQUEST\"")
        body must contain("limit")
      }
    }

    "rejects missing pagination parameters" >> {
      val routes = buildRoutes[IO]()
      val request = Request[IO](Method.GET, Uri.unsafeFromString(s"$v1/posts"))
