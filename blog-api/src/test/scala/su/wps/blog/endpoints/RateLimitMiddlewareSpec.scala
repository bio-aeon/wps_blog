...
class RateLimitMiddlewareSpec extends Specification {

  "RateLimitMiddleware" >> {
    "POST requests" >> {
      "allows requests within limit" >> {
        val app = mkRateLimitedApp(maxRequests = 3, windowSeconds = 60)
        val resp = runPost(app)
      ...
      "returns 429 when limit exceeded" >> {
        ...
        runPost(app)
        val resp = runPost(app)
        resp.status mustEqual Status.TooManyRequests
      }
      ...
      "decrements remaining count on successive requests" >> {
        ...
        runPost(app)
        val resp = runPost(app)
        headerValue(resp, "X-RateLimit-Remaining") must beSome("1")
      }
      ...
      "includes Retry-After header on 429" >> {
        ...
        runPost(app)
        val resp = runPost(app)
        headerValue(resp, "Retry-After") must beSome("30")
      }
      ...
      "sets remaining to 0 on 429" >> {
        ...
        runPost(app)
        val resp = runPost(app)
        headerValue(resp, "X-RateLimit-Remaining") must beSome("0")
      }

      "tracks limits per IP independently" >> {
        val app = mkRateLimitedApp(maxRequests = 1, windowSeconds = 60)
        runPostWithIp(app, "1.2.3.4")
        val resp = runPostWithIp(app, "5.6.7.8")
        resp.status mustEqual Status.Ok
      }
    }
    ...
    "OPTIONS requests" >> {
      "are never rate-limited" >> {
        ...
        runPost(app)
        runPost(app)
        val resp = app.run(Request[IO](Method.OPTIONS, uri"/test")).unsafeRunSync()
        resp.status mustEqual Status.Ok
      }
    }
    ...
    "HEAD requests" >> {
      "are never rate-limited" >> {
        ...
        runPost(app)
        runPost(app)
        val resp = app.run(Request[IO](Method.HEAD, uri"/test")).unsafeRunSync()
        resp.status mustEqual Status.Ok
      }
    }
  }
  ...
  private def runPost(app: HttpApp[IO]): Response[IO] =
    app.run(Request[IO](Method.POST, uri"/test")).unsafeRunSync()

  private def runGet(app: HttpApp[IO]): Response[IO] =
    app.run(Request[IO](Method.GET, uri"/test")).unsafeRunSync()

  private def runPostWithIp(app: HttpApp[IO], ip: String): Response[IO] =
    app
      .run(
