...
final class RoutesImpl[F[_]: Monad] private (postService: PostService[F])
  ...
  private object IntQueryParamMatcher extends QueryParamDecoderMatcher[Int]("limit")

  val routes: HttpRoutes[F] = HttpRoutes.of[F] {
    case GET -> Root / "posts" :? IntQueryParamMatcher(limit) +& IntQueryParamMatcher(offset) =>
